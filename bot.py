...
async def start(chat: Chat, match):
    ...
    session.add(user)
    session.commit()
    await chat.send_text("Дякуємо! Ви успішно підписались на агроновини!")
...
async def get_news(chat: Chat, match):
    ...
    for i in session.query(ParsedItem).all():
        ...
        private.send_text(json.loads(i.data)['title'].encode('l1').decode() + f" ({json.loads(i.data)['link']}", **{"parse_mode": "HTML"})
